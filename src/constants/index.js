//WIP: Do en version of the site
export const freeProjects = [
    {
        src: '/images/landing/3.webp',
        path: '/assets/free-projects/comming-soon.webm',
        alt: 'project1',
        title: 'E-Kurz: První webovky',
        href: '/'
    },
    {
        src: '/images/landing/2.webp',
        path: '/assets/free-projects/comming-soon.webm',
        alt: 'project2',
        title: 'E-Kurz: SEO Začátečník',
        href: '/'
    },
    {
        src: '/images/landing/4.webp',
        path: '/assets/free-projects/comming-soon.webm',
        alt: 'project3',
        title: 'E-Book: Vytvoření nabídky',
        href: '/'
    }
]

export const freeProjectsEN = [
    {
        src: '/images/landing/3.webp',
        path: '/assets/free-projects/comming-soon.webm',
        alt: 'project1',
        title: 'E-Course: First Website',
        href: '/'
    },
    {
        src: '/images/landing/2.webp',
        path: '/assets/free-projects/comming-soon.webm',
        alt: 'project2',
        title: 'E-Course: SEO Starter',
        href: '/'
    },
    {
        src: '/images/landing/4.webp',
        path: '/assets/free-projects/comming-soon.webm',
        alt: 'project3',
        title: 'E-Book: Creating an Offer',
        href: '/'
    }
]

export const cards = [
    {
        title: "Web Design",
        points: [
            "Autentický, Profesionální Design",
            "Aktualizování obsahu zdarma",
            "Pokročilé animace s rychlostí načítání",
            "Nejpřístupnější hosting s doménou",
            "2-4 měsíce dodací lhůta",
        ],
        text: 'Při nesplnění:',
        quarentees: [
            "Vám vrátím 10% z ceny",
            "A 20% slevu na další práci na webu",
        ],
    },
    {
        title: "SEO Optimization",
        points: [
            "Keyword výzkum a správné praktiky",
            "Ac, BP a SEO nad 90% ohodnocení",
            "Týden./Měs. zpráva výkonu vašich stránek",
            "Principy pro zvýšení organické návštěvnosti",
        ],
        text: 'Při nesplnění:',
        quarentees: [
            "Měsíc mých služeb zdarma dokud nebude výsledek",
            "A vrátím Vám 10% z ceny",
        ],
    },
    {
        title: "Marketing Offer",
        points: [
            "Principy při výzkumu a tvorbě nabídky",
            "Vytvoření samotné nabídky, která prodává",
            "Zlepšení prodejů a zisku na vašem webu za první 3 měsíce",
            "První 3 měsíce konzulací zdarma",
        ],
        text: 'Při nesplnění:',
        quarentees: [
            "Vám vrátím 10% z ceny",
            "Měsíc mých služeb zdarma dokud nebude výsledek",
        ],
    },
];

export const cardsEN = [
    {
        title: "Web Design",
        points: [
            "Authentic, Professional Design",
            "FREE Content Updates",
            "Advanced animation with fast loading",
            "Convinient hosting with a domain",
            "2-4 months deadline delivery",
        ],
        text: 'If unfulfilled:',
        quarentees: [
            "10% of price returned",
            "A 20% discount for another job regarding website",
        ],
    },
    {
        title: "SEO Optimization",
        points: [
            "Keyword research and correct practices",
            "Ac, BP a SEO above 90% ratings",
            "Weekly/Monthly resports of your site performance",
            "Priciples to increase organic traffic",
        ],
        text: 'If unfulfilled:',
        quarentees: [
            "A month of my service for free until results are met",
            "10% of price returned",
        ],
    },
    {
        title: "Marketing Offer",
        points: [
            "Principles for reasearch and offer creation",
            "Creating offer, that sells",
            "Improvements in sales and profits withing first 3 months",
            "First 3 months of consulation for FREE",
        ],
        text: 'If unfulfilled:',
        quarentees: [
            "10% of price returned",
            "A month of my service for free until results are met",
        ],
    },
];


export const NewestProjects = [
    {
        title: "Brainwave",
        date: "2023",
        src: "/assets/images/projects/brainwave.webp",
        link: '/projects/brainwave',
        type: 'web',
        alt: 'project2'
    },
    {
        title: "Komponenty",
        date: "2024",
        src: "/assets/images/projects/components.webp",
        link: '/projects/components',
        type: 'web',
        alt: 'project1'
    },
    {
        title: "Apple Web",
        date: "2024",
        src: "/assets/images/projects/apple-site.webp",
        link: '/projects/apple-site',
        type: 'web',
        alt: 'project4'
    },
    {
        title: "E-commerce",
        date: "2023",
        src: "/assets/images/projects/ecom.webp",
        link: '/projects/e-comm',
        type: 'app',
        alt: 'project3'
    },
]


export const NewestProjectsEN = [
    {
        title: "Brainwave",
        date: "2023",
        src: "/assets/images/projects/brainwave.webp",
        link: '/projects/brainwave',
        type: 'web',
        alt: 'project2'
    },
    {
        title: "Components",
        date: "2024",
        src: "/assets/images/projects/components.webp",
        link: '/projects/components',
        type: 'web',
        alt: 'project1'
    },
    {
        title: "Apple Website",
        date: "2024",
        src: "/assets/images/projects/apple-site.webp",
        link: '/projects/apple-site',
        type: 'web',
        alt: 'project4'
    },
    {
        title: "E-commerce",
        date: "2023",
        src: "/assets/images/projects/ecom.webp",
        link: '/projects/e-comm',
        type: 'app',
        alt: 'project3'
    },
]

export const projects = [
    {
        title: "Komponenty",
        date: "2024",
        src: "/assets/images/projects/components.webp",
        href: '/projects/components',
        type: 'web',
        index: 0,
        alt: 'project1',
    },
    {
        title: "Brainwave",
        date: "2023",
        src: "/assets/images/projects/brainwave.webp",
        href: '/projects/brainwave',
        type: 'web',
        index: 1,
        alt: 'project2',
    },
    {
        title: "E-commerce",
        date: "2023",
        src: "/assets/images/projects/ecom.webp",
        href: '/projects/e-comm',
        type: 'app',
        index: 2,
        alt: 'project3',
    },
    {
        title: "Apple Web",
        date: "2024",
        src: "/assets/images/projects/apple-site.webp",
        href: '/projects/apple-site',
        type: 'web',
        index: 3,
        alt: 'project4'
    },
    {
        title: "Portfolio DS",
        date: "2024",
        src: "/assets/images/projects/denisclone.webp",
        href: '/projects/denis-clone',
        type: 'web',
        index: 4,
        alt: 'project5'
    },
    {
        title: "3D Portfolio",
        date: "2024",
        src: "/assets/images/projects/3dpc.webp",
        href: '/projects/3d-pc',
        type: 'web',
        index: 5,
        alt: 'project6'    },
    {
        title: "3D Mlýn",
        date: "2023",
        src: "/assets/images/projects/3dwindmill.webp",
        href: '/projects/3d-windmill',
        type: 'web',
        index: 6,
        alt: 'project7'    },
    {
        title: "3D T-shirt",
        date: "2023",
        src: "/assets/images/projects/3dtshirt.webp",
        href: '/projects/3d-tee',
        type: 'web',
        index: 7,
        alt: 'project8'    },
    {
        title: "Framer Website",
        date: "2024",
        src: "/assets/images/projects/framer.webp",
        href: '/projects/framer',
        type: 'web',
        index: 8,
        alt: 'project9'    },
    {
        title: "Smooth Scroll",
        date: "2024",
        src: "/assets/images/projects/smoothscroll.webp",
        href: '/projects/smooth-scroll',
        type: 'web',
        index: 9,
        alt: 'project10'    },
    {
        title: "Sushi Restaurace",
        date: "2023",
        src: "/assets/images/projects/sushi.webp",
        href: '/projects/sushi',
        type: 'web',
        index: 10,
        alt: 'project11'
    },
    {
        title: "NFT Marketplace",
        date: "2023",
        src: "/assets/images/projects/nft.webp",
        href: '/projects/nft',
        type: 'app',
        index: 11,
        alt: 'project12'
    },
    {
        title: "Blog",
        date: "2024",
        src: "/assets/images/projects/blog.webp",
        href: '/projects/blog',
        type: 'app',
        index: 12,
        alt: 'project13'
    },
    {
        title: "Agentura",
        date: "2024",
        src: "/assets/images/projects/agency.webp",
        href: '/projects/agency',
        type: 'web',
        index: 13,
        alt: 'project14'
    },
    {
        title: "Banka",
        date: "2023",
        src: "/assets/images/projects/bank.webp",
        href: '/projects/bank',
        type: 'web',
        index: 14,
        alt: 'project15'
    },
    {
        title: "Půjčovna Aut",
        date: "2023",
        src: "/assets/images/projects/car.webp",
        href: '/projects/car-app',
        type: 'app',
        index: 15,
        alt: 'project16'
    },
    {
        title: "Cestovní Web",
        date: "2023",
        src: "/assets/images/projects/travel.webp",
        href: '/projects/travel',
        type: 'web',
        index: 16,
        alt: 'project17'
    }
]

export const NewestProjectEN = [
    {
        title: "Brainwave",
        date: "2023",
        src: "/assets/images/projects/brainwave.webp",
        link: '/projects/brainwave',
        type: 'web',
        alt: 'project2'
    },
    {
        title: "Komponenty",
        date: "2024",
        src: "/assets/images/projects/components.webp",
        link: '/projects/components',
        type: 'web',
        alt: 'project1'
    },
    {
        title: "Apple Web",
        date: "2024",
        src: "/assets/images/projects/apple-site.webp",
        link: '/projects/apple-site',
        type: 'web',
        alt: 'project4'
    },
    {
        title: "E-commerce",
        date: "2023",
        src: "/assets/images/projects/ecom.webp",
        link: '/projects/e-comm',
        type: 'app',
        alt: 'project3'
    },
]

export const projectsEN = [
    {
        title: "Components",
        date: "2024",
        src: "/assets/images/projects/components.webp",
        href: '/projects/components',
        type: 'web',
        index: 0,
        alt: 'project1',
    },
    {
        title: "Brainwave",
        date: "2023",
        src: "/assets/images/projects/brainwave.webp",
        href: '/projects/brainwave',
        type: 'web',
        index: 1,
        alt: 'project2',
    },
    {
        title: "E-commerce",
        date: "2023",
        src: "/assets/images/projects/ecom.webp",
        href: '/projects/e-comm',
        type: 'app',
        index: 2,
        alt: 'project3',
    },
    {
        title: "Apple Website",
        date: "2024",
        src: "/assets/images/projects/apple-site.webp",
        href: '/projects/apple-site',
        type: 'web',
        index: 3,
        alt: 'project4'
    },
    {
        title: "Portfolio DS",
        date: "2024",
        src: "/assets/images/projects/denisclone.webp",
        href: '/projects/denis-clone',
        type: 'web',
        index: 4,
        alt: 'project5'
    },
    {
        title: "3D Portfolio",
        date: "2024",
        src: "/assets/images/projects/3dpc.webp",
        href: '/projects/3d-pc',
        type: 'web',
        index: 5,
        alt: 'project6'    },
    {
        title: "3D Windmill",
        date: "2023",
        src: "/assets/images/projects/3dwindmill.webp",
        href: '/projects/3d-windmill',
        type: 'web',
        index: 6,
        alt: 'project7'    },
    {
        title: "3D T-shirt",
        date: "2023",
        src: "/assets/images/projects/3dtshirt.webp",
        href: '/projects/3d-tee',
        type: 'web',
        index: 7,
        alt: 'project8'    },
    {
        title: "Framer Website",
        date: "2024",
        src: "/assets/images/projects/framer.webp",
        href: '/projects/framer',
        type: 'web',
        index: 8,
        alt: 'project9'    },
    {
        title: "Smooth Scroll",
        date: "2024",
        src: "/assets/images/projects/smoothscroll.webp",
        href: '/projects/smooth-scroll',
        type: 'web',
        index: 9,
        alt: 'project10'    },
    {
        title: "Sushi Restaurant",
        date: "2023",
        src: "/assets/images/projects/sushi.webp",
        href: '/projects/sushi',
        type: 'web',
        index: 10,
        alt: 'project11'
    },
    {
        title: "NFT Marketplace",
        date: "2023",
        src: "/assets/images/projects/nft.webp",
        href: '/projects/nft',
        type: 'app',
        index: 11,
        alt: 'project12'
    },
    {
        title: "Blog",
        date: "2024",
        src: "/assets/images/projects/blog.webp",
        href: '/projects/blog',
        type: 'app',
        index: 12,
        alt: 'project13'
    },
    {
        title: "Agency",
        date: "2024",
        src: "/assets/images/projects/agency.webp",
        href: '/projects/agency',
        type: 'web',
        index: 13,
        alt: 'project14'
    },
    {
        title: "Bank",
        date: "2023",
        src: "/assets/images/projects/bank.webp",
        href: '/projects/bank',
        type: 'web',
        index: 14,
        alt: 'project15'
    },
    {
        title: "Car Rental",
        date: "2023",
        src: "/assets/images/projects/car.webp",
        href: '/projects/car-app',
        type: 'app',
        index: 15,
        alt: 'project16'
    },
    {
        title: "Travel Website",
        date: "2023",
        src: "/assets/images/projects/travel.webp",
        href: '/projects/travel',
        type: 'web',
        index: 16,
        alt: 'project17'
    }
]


export const navbarLinks = [
    {
      title: 'Projekty',
      href: '/projects'
    },
    {
      title: 'O mně',
      href: '/about'
    },
    {
      title: 'Kontakt',
      href: '/contact'
    },
    {
      title: 'Materiály',
      href: '/materials'
    }
]

export const navbarLinksEN = [
    {
      title: 'Projects',
      href: '/projects'
    },
    {
      title: 'About',
      href: '/about'
    },
    {
      title: 'Contact',
      href: '/contact'
    },
    {
      title: 'Materials',
      href: '/materials'
    }
]

export const footerLinks = [
    {
        title: 'LinkedIn',
        href: 'https://www.linkedin.com/in/mat%C4%9Bj-forejt-aba4a0221/',
    },
    {
        title: 'Facebook',
        href: 'https://www.facebook.com/forejt.matej/',
    },
    {
        title: 'Instagram',
        href: 'https://www.instagram.com/aether_matejforejt/',
    }
]